...
- [x] 情绪历史列表，支持长按删除
- [x] 12. 今日页面日历功能
  - 新增 Calendar 组件（components/Calendar.tsx）— 自绘月历，支持月份切换、日期标记点
  - 新增 calendarUtils.ts — 月历数据生成、日期格式化工具
  - 新增 checkins 表 + checkinRepository — 每日打卡、连续打卡天数统计
  - 今日页面集成日历、打卡按钮、连续打卡天数、选中日期记录展示
  - 日历标记：记录日=粉色、情绪日=紫色、打卡日=绿色
  - 点击日历日期展示当天记录列表
